...
import {
  buildCompletedResearchCenter,
  countMapBuildingSprites,
  countRegionsWithMapStructures,
  createSlotsAlert,
  expectBuildPaletteNoInternalOverlap,
  expectCanvasNonBlank,
  expectHudNoMajorOverlap,
  expectMapZoomOverlayHidden,
  expectMapZoomOverlayVisible,
  expectPointNotCoveredByHud,
  findSceneText,
  openConceptGame,
  openGame,
  openLiveGame,
  openRegionBuildingsTab,
  progressVariable,
  regionPanelMetrics,
  researchCardReadabilityMetrics,
  researchStatusProgress,
  type Rect
} from "./egrid.helpers";
...
test.describe("E-Grid 2045 web game visuals", () => {
  ...
  test("research tab keeps compact cards readable and scrolls from card surfaces", async ({ page }, testInfo) => {
    ...
    expect(defaultMetrics.titleOverflowMax).toBeLessThanOrEqual(textOverflowTolerancePx);
  ...
  test("map supports wheel zoom and drag panning", async ({ page }, testInfo) => {
    await openGame(page, 1600, 900);
    ...
    await expectMapZoomOverlayHidden(page);
    const before = await page.evaluate(() => window.__EGRID__?.scene.getRegionScreenPoint("fr_nord"));
    expect(before).toBeTruthy();
    await page.mouse.move(anchor.x, anchor.y);
    await page.mouse.wheel(0, -700);
    await page.waitForTimeout(120);
    const zoomed = await page.evaluate(() => window.__EGRID__?.scene.getRegionScreenPoint("fr_nord"));
    expect(zoomed).toBeTruthy();
    const zoomLabel = await findSceneText(page, /^Zoom \d+%$/);
    expect(zoomLabel?.text).not.toBe("Zoom 100%");
    await expectMapZoomOverlayVisible(page);

    const zoomDelta = Math.hypot((zoomed?.x ?? 0) - (before?.x ?? 0), (zoomed?.y ?? 0) - (before?.y ?? 0));
    expect(zoomDelta).toBeGreaterThan(3);

    await page.mouse.move(anchor.x, anchor.y);
    await page.mouse.down();
    await page.mouse.move(anchor.x + 90, anchor.y + 45, { steps: 6 });
    await page.mouse.up();
    await page.waitForTimeout(120);
    const panned = await page.evaluate(() => window.__EGRID__?.scene.getRegionScreenPoint("fr_nord"));
    expect(panned).toBeTruthy();
    expect((panned?.x ?? 0) - (zoomed?.x ?? 0)).toBeGreaterThan(20);
    expect((panned?.y ?? 0) - (zoomed?.y ?? 0)).toBeGreaterThan(10);

    const resetButton = await findSceneText(page, "Reinit.");
    expect(resetButton).toBeTruthy();
    if (resetButton) {
      await page.mouse.click(resetButton.x + resetButton.width / 2, resetButton.y + resetButton.height / 2);
    }
    await page.waitForTimeout(120);
    await expectMapZoomOverlayHidden(page);

    await page.screenshot({ path: testInfo.outputPath("map-zoom-pan.png"), fullPage: true });
  });
});
